class WalletController < ApplicationController
  ...
  def login
    if params[:challenge_code].present?
      ...
      session[:Wallet_device_token] = nil unless success
    end
  ...
  def dividends
    ...
    @dividends.each do |dividend|
      dividend["instrument"] = Wallet_get dividend["instrument"]
    end
  end
    ...
    if params["symbols"].present?
      ...
      begin
        ...
        if(!@quotes || @quotes.empty?)
          ...
          @quotes = @instruments.map{|instrument| Wallet_get instrument["quote"]}
        end
      ensure
        @quotes ||= {}
      end
  ...
  def new_transfer
    response = Wallet_post "https://api.Wallet.com/ach/transfers/", {direction: params[:direction].downcase, amount: params[:amount], ach_relationship: params[:ach_relationship]}
  ...
  def cancel_transfer
    response = Wallet_post params["transfer_url"], {}
  ...
  def positions
    ...
    @investments.each do |symbol,data|
      ...
      get_splits data["instrument"].Wallet_id
      ...
      #data["quantity"] = current_shares_owned # override quantity from Wallet
  ...
  def reorder_positions
    ...
    if list.present?
      Wallet_instruments = Instrument.find(params[:Wallet_order]).pluck(:Wallet_id)
      if list.group =~ /crypto_watchlist_(.+)/i
        name = list.name || $1
        get_crypto_watchlists
        crypto_list = @crypto_watchlists.find{|l| l["name"] == name}
        response = set_crypto_watchlist crypto_list["id"], instrument_order.pluck(:Wallet_id)
      elsif list.group == "portfolio"
        reorder_portfolio_positions Wallet_instruments
      else
        reorder_watchlist list.group, Wallet_instruments
      end
  ...
  def add_to_watchlist
    if params[:type].present? && params[:type] =~ /crypto/i
      ...
      response = set_crypto_watchlist watchlist["id"], (watchlist["currency_pair_ids"] << params[:Wallet_id])
    else
  ...
  def remove_from_watchlist
    ...
    if stock_list.cryptocurrency_list?
      ...
      response = set_crypto_watchlist watchlist["id"], watchlist["currency_pair_ids"].select{|id| id != params["Wallet_id"]}
    else
  ...
  def orders
    ...
    @orders.each do |order|
      order["instrument"] = Wallet_get order["instrument"]
  ...
  def cancel_order
    response = Wallet_post params["url"], {}
    success = response.empty?
    if success
  ...
  def api
    @url = params["url"] || 'https://api.Wallet.com/'
